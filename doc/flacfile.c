...
get_mime_type (const char *filename)
{
  ...
  if (mime_type == NULL)
    {
      fprintf (stderr,
               "flacfile: Unable to determine MIME type automatically; please specify one.\n");
    }
...
new_block (const char *filename, const char *description, const char *mime)
{
  ...
  if (desc_len > 255 || mime_len > 255)
    {
      fprintf (stderr,
               "flacfile: Description and MIME type must both be shorter than 256 characters.\n");
      exit (1);
    }
...
usage ()
{
  ...
  fprintf (stderr,
           "Usage: flacfile add <FLAC filename> <description> <filename to attach> [<MIME type>]\n");
  fprintf (stderr, "       flacfile list <FLAC filename>\n");
  fprintf (stderr,
           "       flacfile extract <FLAC filename> <description> <filename to save as> [<MIME type>]\n");
  fprintf (stderr,
           "       flacfile remove <FLAC filename> <description> [<MIME type>]\n");
  exit (1);
}
  ...
  if (strcmp (argv[1], "add") == 0)
    {
      ...
      if (strcmp (argv[2], argv[4]) == 0)
        {
          fprintf (stderr,
                   "flacfile: You didn't really want to attach a file to itself.\n");
          exit (1);
        }
  ...
  else if (strcmp (argv[1], "extract") == 0)
    {
      ...
      if (strcmp (argv[2], argv[4]) == 0)
        {
          fprintf (stderr,
                   "flacfile: You didn't really want to extract a file onto itself.\n");
          exit (1);
        }
